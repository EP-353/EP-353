...
//Compile with:
//clang 02.AsymmetricSoftClipping.c -o 02.AsymmetricSoftClipping -lsndfile
//Run with:
//./02.AsymmetricSoftClipping

#define kInputFileName "Guitar.wav"
#define kOutputFileName "AsymmetricSoftClipping.wav"
#define kGain 10.0
#define kLevel 0.5
...
void process(float *inBuffer, float *outBuffer, sf_count_t bufferSize){
  ...
  for(sf_count_t n = 0; n < bufferSize; n++){
    ...
    if(outBuffer[n] >= 0.0f){
      outBuffer[n] = atan(outBuffer[n]) / (M_PI / 2.0f); //Soft clipping with arctangent
    } else if(outBuffer[n] < 0.0f){
      outBuffer[n] = atan(outBuffer[n]) / (M_PI / 2.0f) * 0.1f; //Introduce asymmetry in negative signals
    }
